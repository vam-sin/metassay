...
def process_chrom(ch):
    ...
    for task in tqdm(task_names, desc='Loading Task Data'):
        chrom_data[task] = np.load(f'encode_dataset/proc_3k/binTasksSep/{task}_{ch}.npz', allow_pickle=True)['arr_0'][()]
    ...
    for i in tqdm(range(0, len(bin_regions_by_chr[ch]), chunk_size), desc=f'Processing {task} {ch}'):
        ...
        for bin_region in chunk_bins:
            ...
            chunk_data[bin_region]['dna_seq'] = one_hot_encode(chrom_data[task_names[0]][bin_region]['dna_seq'], seq_len=3000) 
